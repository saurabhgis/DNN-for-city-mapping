    ...
    for i, color in enumerate(colors):
        ...
        for i in range(1,5):
            ...
            if "person" in caption:
                folium.Marker(
                    location=[float(resultsg.metadata[i]['location']['lat'])+ 0.000001 * int(xx), float(resultsg.metadata[i]['location']['lng']) + 0.000001 * int(yy)],
                    popup=caption,
...
def test_simple(params):
    ...
    result = api.query("""
        way(50.075,14.417,50.075,14.421) ["highway"];
     (._;>;);
        out body;
        """)
    location1 = []
